...
function loadDatasetsForStates(rawData, selectedCountries, datasetGen, stateAccessor) {
    ...
    return Object.fromEntries(selectedCountries
        .map(countryName => {
...
function loadBasketballDataset(draw, numTopWords, selectedCountries) {
    ...
    d3.json(path, (error, data) => {
        ...
        let datasetGen = getDatasetGenerator();
        categories = getUniqueValues(rawData.map(datasetGen.categoryAccessor));
        totalDataMulti = loadDatasetsForStates(rawData, selectedCountries,
            datasetGen, getStateAccessor());
