...
function compareSuddenAttentionMeasure(word1, word2) {
    return (word1, word2) => word2.sudden - word1.sudden;
}
...
class DatasetGenerator {
    ...
    processListOfRecords(records) {
        //TODO performance probably could maybe be optimized by instantiating map objects first
        //TODO should check if words REALLY are unique within timestep, category groups
        // for entries with state association, right now they are NOT!
        ...
        /*return new Dataset(
            new Map(timesteps.map((timestep, timestepIndex) => {
                const recordsForTimestep = records.filter(record => this.timestepAccessor(record));

                const recordsForCategoryAndTimestep ;

                const transformedRecordsForTimestepAndCategory = categories
                    .map(category => [category, recordsForTimestep
                        .filter(record => this.categoryAccessor(record) === category)
                        .map(record => this.mapRecord(record))])
                return [timestep, new Map(transformedRecordsForTimestepAndCategory)];
            })),
            this
        );*/
    }
...
function loadDataset(rawData, selectedCountries, datasetGen, stateAccessor) {

    const states = getUniqueValues(rawData.map(entry => stateAccessor(entry)));
    categories = getUniqueValues(rawData.map(record => datasetGen.categoryAccessor(record)));
...
function getFilterFunc(datasetName = undefined) {
    ...
    let filterFuncs = {
        "Basketball": entry => entry.date < 2006,
        "UCD": entry => entry.date < 2006,
...
function loadBasketballDataset(draw, numTopWords, selectedCountries) {
    ...
    d3.json(path, (error, data) => {
        ...
        let rawData = data["per_state_counts"];

        const states = getUniqueValues(rawData.map(record => record.state));
        categories = getUniqueValues(rawData.map(record => record.position));

        const datasetGen = DatasetGenerator.create()
        ...
        let datasetsPerCountry = loadDataset(rawData, selectedCountries,
            datasetGen, (record) => record["state"]);

        totalDataMulti = Object.fromEntries(datasetsPerCountry);
...
function loadDiseasesDataset(draw, numTopWords, selectedCountries) {
    let path = "data/nndss_2016_2020_clean.txt";
    d3.tsv(path, function (error, rawData) {
        ...
        let datasetsPerCountry = loadDataset(rawData, selectedCountries,
            datasetGen, (record) => record["States"]);
